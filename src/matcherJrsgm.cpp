...
void MatcherJrSGM::compute(cv::Mat left_image, cv::Mat right_image, cv::Mat disp)
{
  ...
  {
    ROS_ERROR("exception computing stereo");
...
void MatcherJrSGM::enableInterpolation(bool enable)
{
  ...
  //ROS_INFO("cycling pyramid params");
  for (auto &pyramid : params.oPyramidParams)
  {
...
JR::Phobos::SMatchingParameters MatcherJrSGM::parsePyramid(iniReader *settings, std::string group)
{
  ...
  pyramid.nNth = str_to_int(settings->value(group, "Interpolator Number Nth Elements", "32"));          // "Interpolator Number Nth Elements"
  ...
  pyramid.fRadius = str_to_int(settings->value(group, "Interpolator Match Radius", "-1")); // "Interpolator Match Radius"
  ...
  pyramid.oMetricParams.fScaleStepSize = str_to_float(settings->value(group, "Feature Set Slanted Window Scale Step Size", "0.250000"));
  ...
  pyramid.oMetricParams.fSlantStepSize = str_to_float(settings->value(group, "Feature Set Slanted Window Shift Step Size", "0.250000"));
...
void MatcherJrSGM::setConfig()
{
  ...
  ROS_INFO("re-created stereo handle");
  try
  {
    matcher_handle = JR::Phobos::CreateMatchStereoHandle(params);
  }
  ...
  {
    ROS_ERROR("exception creating match stereo handle");
    std::cout << ex.what() << std::endl;
  }
}